...
## Task 18 verification outcome (2026-04-20)

### Metrics

| Metric | First tick | Second tick |
|---|---|---|
| Wall-clock duration | **37.79 s** | **0.74 s** |
| Temporal chat workflows listed | 2 (full retention) | 2 (since-filtered, both still running) |
| Temporal generation-batch listed | 19 (full retention) | 1 (since-filtered, only the running one) |
| Observer non-terminal union | 0 (empty DB) | 1 (the running batch itself) |
| Root workflows ingested | 21 | 3 (2 chat + 1 gen-batch running) |
| Descendants processed via recursion | ~1044 | 10 (only still-running children; terminal children skipped) |
| Workflows in observer (end) | 1067 | 1067 |
| cgw rows | 19 (17 enriched, 2 orphans WARN-logged) | 19 (0 new, 2 orphans still WARN-logged) |
| Errors / tracebacks | 9 `RPCError: history not found, may have passed retention period` for orphan children (handled — per-workflow try/except, sync continued) | 0 |

### Behaviour confirmed

- **Running workflow visible**: `generation-batch-76e68bfe-a5b9-48b0-b0f5-9e4f29ff719d` (status `RUNNING` in Temporal) now exists in `workflows` table with `status='RUNNING'`. This was completely invisible pre-v2 due to the non-terminal skip.
- **Child statuses are not uniformly RUNNING**: children split across `COMPLETED` (1021), `FAILED` (15), `RUNNING` (10 — belong to the one running batch). Pre-v2 had 10 children stuck at `RUNNING` from TERMINATED parents; the new parser attributes statuses correctly on fresh ingest.
- **Orphan logging**: 2 orphan batches (`35a23b86-…`, `e9fbdbe1-…`) whose `batch_id` has no match in app DB — logged at WARN each tick, not deleted. Expected per spec.
- **Watermarks advance correctly**: `chat_workflows` and `generation_batch_workflows` entries in `sync_state`. Second tick's `Listing … (since <timestamp>)` confirms they're being passed to Temporal.
- **Batched terminal skip in recursion**: second tick's generation sync processed 11 items (1 root + 10 running children) rather than re-fetching histories for all 1021 completed children. That's where the 50× speedup mostly comes from.
- **Per-phase isolation**: no phase's failure would kill the sync (not exercised this run — no transient failures — but verified by code review).
- **Log file**: `logs/sync.log` (25 KB after two ticks) with both ticks captured, rotating config confirmed (`10 MB × 5` backups).

### Known-acceptable artifacts

- 9 `RPCError: history not found` tracebacks in first-tick log. These come from recursing into children of the two orphan batches whose history Temporal has aged out. The per-workflow try/except catches them and sync continues; no data loss. Follow-up opportunity: catch `RPCError` specifically and log a single WARN line rather than a full traceback (cosmetic).
- 10 `SCHEDULED` activities and 10 `RUNNING` children, all belonging to the live running batch. These are genuinely in-flight — they're not the "stuck" SCHEDULED/RUNNING rows from the pre-v2 diagnosis (those were in a now-dropped DB).

### Conclusion

All three user-reported symptoms addressed:
1. **Running column creators visible** — ✅ (first tick confirms).
2. **Syncs take long** — ✅ (50× speedup on ticks after the first, bounded by non-terminal workflow count).
3. **Sync sometimes misses things** — ✅ (visibility-lag union + NULL-metadata backfill + watermark-race fix).
